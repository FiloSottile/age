...
or the predicate of a pub key file in ~/.config/age ("me", "john", "marysmith")
...
If no key is provided along with the --decrypt flag, ~/.config/age will be scanned
for files with the .key extension as well as ~/.ssh/id_rsa and ~/.ssh/id_ed25519

Examples:
    $ age-keygen
    /home/myuser/.config/age/ does not exist, creating
    Public key: age1prxppd3gy8flrlfcxnk7wshxuer75wcc7seq6n9l9ctuxjggvggs48qvzn
    Created at: 2020-02-21T18:35:46-05:00
    /home/myuser/.config/age/me.pub and /home/dkovach/.config/age/me.key written
    ...
    /home/myuser/.config/age/john.pub and /home/dkovach/.config/age/john.key written
    $ tar cvz ~/data | age -r age1zhan38xvccu9sa868xhtmpelxmacwtfdwq5d4048mzh06s4cuazs8mqny3 > data.tar.gz.age
    $ age -d -i ~/.config/age/john.key -o data.tar.gz data.tar.gz.age`
...
const publicKeySizeLimit = 1 << 24 // 16 MiB

func encryptKeys(keys []string, in io.Reader, out io.Writer, armor bool) {
	cfgdir, err := os.UserConfigDir()
	if err != nil {
		logFatalf("could not find UserConfigDir (perhaps $HOME is not defined?): %v", err)
	}
	agedir := cfgdir + "/age/"
	...
	for _, arg := range keys {
		fname := agedir + arg
		fpname := fname + ".pub"
		if _, err := os.Stat(fpname); os.IsNotExist(err) {
			...
			contents, err := ioutil.ReadAll(io.LimitReader(f, publicKeySizeLimit))
			if err != nil {
				logFatalf("failed to read %q: %v", fpname, err)
			}
			if len(contents) == publicKeySizeLimit {
				logFatalf("failed to read %q: file too long", fpname)
			}
...
func decrypt(keys []string, in io.Reader, out io.Writer) {
	...
	// scan ~/.age/ for files with the .key extension and parse
  cfgdir, _ := os.UserConfigDir()
	keyglob := cfgdir + "/age/*.key"
	matches, _ := filepath.Glob(keyglob)
	for _, m := range matches {
		keys = append(keys, m)
	}

	// scan for ssh keys in standard locations
	homedir, _ := os.UserHomeDir()
	idrsa := homedir + "/.ssh/id_rsa"
	ided := homedir + "/.ssh/id_ed25519"
	if _, err := os.Stat(idrsa); err != nil {
		keys = append(keys, idrsa)
	}
	if _, err := os.Stat(idrsa); err != nil {
		keys = append(keys, ided)
	}
